...
class NWS_Alerts_Entry {
    ...
    */
    public function get_output_text($details = true) {
        ...
        // Size
		if ($details) {
            $return_value = '<span class="nws-alerts-event nws-alerts-color-' . $this->cap_event_slug . '">' . $this->cap_event . '</span><br /> ' . ucwords(strtolower($this->summary)) . '<div>' . $this->cap_effective . '-' . $this->cap_expires . '</div>';
        } else {
